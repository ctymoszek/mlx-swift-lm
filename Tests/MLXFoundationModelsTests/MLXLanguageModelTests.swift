    ...
    struct GrammarErrorMappingTests {
        ...
        func invalidJSONSchemaMapsToTypedError() throws {
            ...
            let mapped = MLXLanguageModel.Executor.mapGrammarError(
                .invalidJSONSchema("xgrammar rejected the schema: top-level type must be a string")
            )

            guard case LanguageModelError.unsupportedGenerationGuide(let payload) = mapped else {
                Issue.record(
